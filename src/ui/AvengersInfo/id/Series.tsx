...
const Series = async ({ id }: { id: number }) => {
    ...
    const creators = series.creators.items.map(creator => {
        return {
            name: creator.name
        }
    })
    const characters = series.characters.items.map(character => {
        return {
            name: character.name
        }
    })
    const stories = series.stories.items.map(story => {
        return {
            name: story.name
        }
    })
    const comics = series.comics.items.map(comic => {
        return {
            name: comic.name
        }
    })
    ...
    return (
        <div className='flex flex-col'>
            <div className='flex items-center justify-center gap-48'>
                <div>
                    <Image src={series.thumbnail.path + '.' + series.thumbnail.extension} height={330} width={330} alt='series' />
                </div>
                {/* Title card */}
                <div className="">
